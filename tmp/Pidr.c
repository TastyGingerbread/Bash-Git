...
void  send(int serial_port, char* mes)
{
	printf("Request:\t%s", mes);
	write(serial_port, mes,  sizeof(mes) * 8);
}

void receive(int serial_port, char* response, char stop_byte)
{
		char* resp;
		*resp = 'v';
		while(*resp != stop_byte)
		{
